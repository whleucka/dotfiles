...
- `:StimRestore[!] [plugin-name]`: Reverts plugins to the revisions pinned in `nvim-pack-lock.json` *without* fetching from the network (a rollback). Pass a plugin name to roll back just one, or omit it to roll back all. Internally calls `vim.pack.update(names, { offline = true, target = "lockfile" })`. Shows a confirmation buffer with the diff — `:write` to apply or `:quit` to discard. Use `:StimRestore!` to skip confirmation. To recover from a bad update: restore the lockfile from version control (e.g. `git checkout` it to a known-good commit), then run `:StimRestore` and confirm.
